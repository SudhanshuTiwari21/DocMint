...
const TOOL_ID = "convert-to-png";
const DAILY_LIMIT = 20;
const ACCEPT = "image/jpeg,image/jpg,image/gif,image/webp,image/bmp,image/tiff,image/tif";
...
export function ConvertToPngTool() {
  ...
  const handleFileSelect = useCallback((fileList: File[] | null) => {
    ...
    if (valid.length !== fileList.length) {
      setError("Some files were skipped. Please use JPG, GIF, WebP, BMP, or TIF images.");
    }
    setFiles(valid);
  }, []);
  ...
  const handleDrop = useCallback(
    (e: React.DragEvent) => {
      ...
      if (f.length) handleFileSelect(f);
    },
    [handleFileSelect]
  );
  ...
  const runConvert = useCallback(async () => {
    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        const blob = await imageToPng(files[i]!);
        const base = files[i]!.name.replace(/\.[^.]+$/, "");
        out.push({ blob, url: URL.createObjectURL(blob), name: `${base}.png` });
        if (!isPremiumUser) {
  ...
  }, [files, isPremiumUser, usage, userId]);
  ...
  return (
    <section className="space-y-6" aria-labelledby="convert-png-heading">
      <h2 id="convert-png-heading" className="sr-only">
        Convert to PNG
      </h2>
      ...
      >
        <label className="flex cursor-pointer flex-col items-center gap-2">
          ...
          <span className="text-xs text-slate-500 dark:text-slate-400">JPG, GIF, WebP, BMP, TIF</span>
          <input
            type="file"
            accept={ACCEPT}
            multiple
        ...
        {files.length > 0 && (
          <div className="mt-4 flex flex-wrap items-center justify-between gap-2 rounded-lg bg-white px-4 py-2 shadow-sm dark:bg-slate-700">
            <span className="text-sm text-slate-700 dark:text-slate-300">{files.length} file(s) selected</span>
            <button
              type="button"
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-neutral-600 dark:bg-neutral-900">
          ...
          <div className="space-y-3">
            {results.map((r, i) => (
              <div
                key={i}
                className="flex flex-wrap items-center justify-between gap-4 rounded-lg border border-slate-200 bg-slate-50 p-3 dark:border-neutral-600 dark:bg-slate-700/50"
              >
            ...
            {results.length > 1 && (
              <button
                ...
                className="mt-2 flex w-full items-center justify-center gap-2 rounded-lg border border-slate-300 px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-100 dark:border-neutral-600 dark:text-slate-300 dark:hover:bg-neutral-800"
              >
                <Download className="h-4 w-4" aria-hidden />
                Download all
              </button>
      ...
      <LimitReachedModal open={showLimitModal} onClose={() => setShowLimitModal(false)} toolName="convert to PNG" />
    </section>
  );
}
